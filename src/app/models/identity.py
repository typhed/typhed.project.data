...
webhook: the shared :class:`UserProfile` table (the cross-project identity anchor,
keyed on the immutable Clerk user id) and the :class:`WebhookEvent` ledger used
for at-least-once delivery de-duplication.
"""
...
class ClerkUserProfile(ORMBase):
    ...
    clerk_id : Mapped[str] = mapped_column(
        String(255), primary_key = True, name = "pk_clerk_id"
    )
...
class WebhookEvent(ORMBase):
    ...
    svix_id : Mapped[str] = mapped_column(
        String(255), primary_key = True, name = "pk_svix_id"
    )
